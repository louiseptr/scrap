...
def get_depute_infos(depute_url,name)
  ...
  page = Nokogiri::HTML(open("http://www2.assemblee-nationale.fr#{depute_url.text}"))
  begin
  ...
  array_name = name.text.split
  first_name=array_name[1]
  last_name=array_name[2..-1].join(' ')
  hash = {"first_name" => first_name, "last_name" => last_name, "email" => email_output}
  #p hash
  #hash
end
...
def perform
  ...
  array_url_names[0].length.times do |i|
    array_all_deputes << get_depute_infos(array_url_names[0][i], array_url_names[1][i])
  end
  array_all_deputes
end
puts perform